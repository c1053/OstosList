...
import kotlinx.android.synthetic.main.activity_lemmiki.*

class lemmiki : AppCompatActivity() {
    val myDbManager = MyDbManager(this)
    val alertdi = AlertD(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_lemmiki)
        this.setTitle("Lemmiki")
    }

    override fun onResume() {
        super.onResume()
        myDbManager.openDb()
        val dataList = myDbManager.readDbData("lemmiki")
    ...
    fun add_to_lemmiki(view: View?) {
        myDbManager.openDb()
        myDbManager.insertToDb(lemmiki_mess.text.toString(), "lemmiki", "0")
        this.lemmiki_mess.setText("")
        val dataList = myDbManager.readDbData("lemmiki")
        btnList.removeAllViews()
        var id = 1
        for (item in dataList){
