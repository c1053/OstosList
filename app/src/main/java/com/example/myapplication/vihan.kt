...
import kotlinx.android.synthetic.main.activity_vihan.*

class vihan : AppCompatActivity() {
    val myDbManager = MyDbManager(this)
    val alertdi = AlertD(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_vihan)
        this.setTitle("Vihanneksia")
    }
    override fun onResume() {
        super.onResume()
        myDbManager.openDb()
        val dataList = myDbManager.readDbData("vihan")
    ...
    fun add_to_vihan(view: View?) {
        myDbManager.openDb()
        myDbManager.insertToDb(vihan_mess.text.toString(), "vihan", "0")
        this.vihan_mess.setText("")
        val dataList = myDbManager.readDbData("vihan")
        btnList.removeAllViews()
        var id = 1
        for (item in dataList){
