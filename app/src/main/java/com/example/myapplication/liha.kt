...
import kotlinx.android.synthetic.main.activity_liha.*
import kotlinx.android.synthetic.main.fordialog.*

class liha : AppCompatActivity() {
    val myDbManager = MyDbManager(this)
    val alertdi = AlertD(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_liha)
        this.setTitle("Liha")
    }
    override fun onResume() {
        super.onResume()
        myDbManager.openDb()
        val dataList = myDbManager.readDbData("liha")
    ...
    fun add_to_liha(view: View?) {
        myDbManager.openDb()
        myDbManager.insertToDb(liha_mess.text.toString(), "liha", "0")
        this.liha_mess.setText("")
        val dataList = myDbManager.readDbData("liha")
        btnList.removeAllViews()
        var id = 1
        for (item in dataList){
