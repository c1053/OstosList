...
import kotlinx.android.synthetic.main.activity_maito.*

class maito : AppCompatActivity() {
    ...
    override fun onResume() {
        super.onResume()
        myDbManager.openDb()
        val dataList = myDbManager.readDbData("maito")
    ...
    fun add_to_maito(view: View?) {
        myDbManager.openDb()
        myDbManager.insertToDb(maito_mess.text.toString(), "maito", "0")
        this.maito_mess.setText("")
        val dataList = myDbManager.readDbData("maito")
        btnList.removeAllViews()
        var id = 1
        for (item in dataList){
