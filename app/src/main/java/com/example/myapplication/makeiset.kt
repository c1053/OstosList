...
import kotlinx.android.synthetic.main.activity_makeiset.*

class makeiset : AppCompatActivity() {
    val myDbManager = MyDbManager(this)
    val alertdi = AlertD(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_makeiset)
        this.setTitle("Makeiset")
    }
    override fun onResume() {
        super.onResume()
        myDbManager.openDb()
        val dataList = myDbManager.readDbData("makeiset")
        ...
        for (item in dataList){
            ...
            btn.setOnClickListener(object: View.OnClickListener {
                override fun onClick(v: View?) {
                    createSimpleDialog(tN, "makeiset")
    ...
    fun add_to_makeiset(view: View?) {
        myDbManager.openDb()
        myDbManager.insertToDb(makeiset_mess.text.toString(), "makeiset", "0")
        this.makeiset_mess.setText("")
        val dataList = myDbManager.readDbData("makeiset")
        btnList.removeAllViews()
        var id = 1
        for (item in dataList){
